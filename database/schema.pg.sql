...
CREATE TABLE clientes (
  ...
  saldo_actual      DECIMAL(14,4) NOT NULL DEFAULT 0 CHECK (saldo_actual >= 0),
...
-- Correcciones manuales de costo. Las compras van en inventario_movimientos.
...
CREATE INDEX ix_pcostos_producto ON producto_costos (producto_id, creado_en);
CREATE INDEX ix_pcostos_usuario  ON producto_costos (usuario_id);
ALTER TABLE producto_costos ADD CONSTRAINT fk_pcostos_producto FOREIGN KEY (producto_id) REFERENCES productos(id)  ON DELETE RESTRICT;
ALTER TABLE producto_costos ADD CONSTRAINT fk_pcostos_sucursal FOREIGN KEY (sucursal_id) REFERENCES sucursales(id) ON DELETE RESTRICT;
ALTER TABLE producto_costos ADD CONSTRAINT fk_pcostos_usuario  FOREIGN KEY (usuario_id)  REFERENCES usuarios(id)   ON DELETE RESTRICT;
...
CREATE TABLE producto_stock (
  ...
  costo_promedio    DECIMAL(14,4) NOT NULL DEFAULT 0 CHECK (costo_promedio >= 0),
  -- FALSE = costo_promedio es la semilla tecleada al crear el producto y la
  -- proxima entrada de mercancia lo pisa; TRUE = ya salio de plata real.
...
-- Plata que sale para pagarle al proveedor una entrada a credito. Es a la compra
-- lo que `abonos` es al credito del cliente: el saldo vive en
-- `compras.saldo_pendiente` y aca queda el detalle de como se fue bajando.
CREATE TABLE compra_pagos (
  id                 BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  compra_id          BIGINT NOT NULL,
  sucursal_id        BIGINT NOT NULL,
  -- Nulo si se pago por transferencia sin caja abierta: no salio de la gaveta.
...
CREATE INDEX ix_cp_compra   ON compra_pagos (compra_id, fecha);
CREATE INDEX ix_cp_sucursal ON compra_pagos (sucursal_id, fecha);
CREATE INDEX ix_cp_turno    ON compra_pagos (turno_caja_id);
CREATE INDEX ix_cp_metodo   ON compra_pagos (metodo_pago_id);
CREATE INDEX ix_cp_usuario  ON compra_pagos (usuario_id);
CREATE UNIQUE INDEX uq_cp_idem ON compra_pagos (clave_idempotencia) WHERE clave_idempotencia IS NOT NULL;
...
CREATE TABLE abonos (
  ...
  tasa_aplicada      DECIMAL(18,6) NOT NULL CHECK (tasa_aplicada > 0),
  tasa_cambio_id     BIGINT,
  monto_usd          DECIMAL(14,4) NOT NULL CHECK (monto_usd > 0),
  monto_aplicado_usd DECIMAL(14,4) NOT NULL DEFAULT 0 CHECK (monto_aplicado_usd >= 0),
  saldo_a_favor_usd  DECIMAL(14,4) NOT NULL DEFAULT 0 CHECK (saldo_a_favor_usd >= 0),
  ...
  -- El vuelto puede salir en la OTRA moneda (paga en Bs y se le devuelve en $
  -- porque no hay billetes chicos), de ahi `cambio_moneda_codigo`.
  monto_recibido_moneda DECIMAL(18,4),
  cambio_moneda         DECIMAL(18,4),
  cambio_moneda_codigo  tipo_metodo_pago_moneda,
  referencia         VARCHAR(60),
...
-- Solo aplica cuando el vuelto se entrego en la MISMA moneda del abono; si salio en
-- la otra, la igualdad se hace contra la tasa y la valida el servicio (un CHECK no
-- puede convertir monedas sin arrastrar el redondeo del piso).
...
CREATE TABLE abono_aplicaciones (
  ...
  monto_aplicado_usd DECIMAL(14,4) NOT NULL CHECK (monto_aplicado_usd > 0),
  creado_en         TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id)
);
...
-- Carritos aparcados en el POS: el cliente todavia no se decide y el cajero
-- atiende al siguiente sin perder lo que el primero llevaba.
--
-- NO son ventas: no consumen consecutivo, no tocan inventario, no reservan stock
-- y no entran en ningun reporte. Son borradores de pantalla, de ahi que el carrito
-- viaje como JSONB y no como tabla de detalle.
--
-- Viven en la base y no en el navegador para sobrevivir a un refresco y para poder
-- retomarse desde otra caja de la misma sucursal.
CREATE TABLE ventas_en_espera (
  id             BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  sucursal_id    BIGINT NOT NULL,
  usuario_id     BIGINT NOT NULL,
  cliente_id     BIGINT,
  -- Con que la llama el cajero: el nombre del cliente, o "el senor de la moto".
  nombre         VARCHAR(80) NOT NULL,
  nota           VARCHAR(200),
  -- Total y renglones al guardar: pintan la lista sin abrir el JSON. La cuenta de
  -- verdad se recalcula al retomar.
  total_usd      DECIMAL(14,2) NOT NULL DEFAULT 0 CHECK (total_usd >= 0),
...
CREATE INDEX ix_espera_sucursal ON ventas_en_espera (sucursal_id, creado_en DESC);
ALTER TABLE ventas_en_espera ADD CONSTRAINT fk_espera_sucursal FOREIGN KEY (sucursal_id) REFERENCES sucursales(id) ON DELETE RESTRICT;
ALTER TABLE ventas_en_espera ADD CONSTRAINT fk_espera_usuario  FOREIGN KEY (usuario_id)  REFERENCES usuarios(id)   ON DELETE RESTRICT;
-- SET NULL y no RESTRICT: borrar un cliente no puede quedar bloqueado por un borrador.
ALTER TABLE ventas_en_espera ADD CONSTRAINT fk_espera_cliente  FOREIGN KEY (cliente_id)  REFERENCES clientes(id)   ON DELETE SET NULL;
...
--
-- Saldo que el banco reporta cada dia, cargado a mano. Tabla AISLADA: ningun
-- otro modulo (POS, caja, reportes, dashboard) la lee ni la escribe.
-- =============================================================================
CREATE TABLE banco_saldos (
  id             BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  sucursal_id    BIGINT NOT NULL,
  fecha          DATE NOT NULL,
  -- Se permiten negativos: una cuenta puede quedar en rojo.
...
CREATE UNIQUE INDEX uq_banco_saldos_dia ON banco_saldos (sucursal_id, fecha);
CREATE INDEX ix_banco_saldos_fecha ON banco_saldos (fecha);
ALTER TABLE banco_saldos ADD CONSTRAINT fk_banco_saldos_sucursal FOREIGN KEY (sucursal_id) REFERENCES sucursales(id) ON DELETE RESTRICT;
ALTER TABLE banco_saldos ADD CONSTRAINT fk_banco_saldos_usuario  FOREIGN KEY (usuario_id)  REFERENCES usuarios(id)   ON DELETE SET NULL;
CREATE TRIGGER trg_banco_saldos_actualizado BEFORE UPDATE ON banco_saldos FOR EACH ROW EXECUTE FUNCTION actualizar_actualizado_en();
